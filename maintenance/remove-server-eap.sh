...
IT_MODULE_DIR="${IT_MODULES_DIR}/eap${VERSION}"
...
if $MIGRATIONS_ONLY; then
    ...
    if [ ! -d "$MIGRATIONS_VERSION_DIR" ] && [ ! -d "$DOCS_VERSION_DIR" ] && [ ! -d "$IT_MODULE_DIR" ]; then
        echo "Error: No migration, doc or integration-test directories found for JBoss EAP ${VERSION}"
...
if grep -q "<module>eap${VERSION}</module>" "$IT_PARENT_POM"; then
    sed -i '' "/<module>eap${VERSION}<\/module>/d" "$IT_PARENT_POM"
    echo "  Removed <module>eap${VERSION}</module>"
else
