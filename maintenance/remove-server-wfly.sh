...
# Script to remove a WildFly server version from the migration tool
# Usage:
#   ./remove-server-wfly.sh 40.0          (removes WildFly 40.0)
...
if [ $# -lt 1 ] || [ $# -gt 2 ]; then
    ...
    echo "Example: $0 40.0"
    echo "Example: $0 40.0 --migrations-only  (removes only migrations and docs)"
    exit 1
fi
...
if ! [[ "$VERSION" =~ ^[0-9]+\.[0-9]+$ ]]; then
    echo "Error: Version must be in major.minor format (e.g., 40.0)"
    exit 1
fi
...
if $MIGRATIONS_ONLY; then
    ...
    if grep -q "<module>servers/wildfly${VERSION}</module>" "$ROOT_POM"; then
        sed -i '' "/<module>servers\/wildfly${VERSION}<\/module>/d" "$ROOT_POM"
...
if [ -d "$MIGRATIONS_VERSION_DIR" ]; then
    for migration_dir in $(ls "$MIGRATIONS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
        if grep -q "<module>migrations/wildfly${VERSION}/${migration_dir}</module>" "$ROOT_POM"; then
            sed -i '' "/<module>migrations\/wildfly${VERSION}\/${migration_dir}<\/module>/d" "$ROOT_POM"
        fi
    done
...
if [ -d "$DOCS_VERSION_DIR" ]; then
    for doc_dir in $(ls "$DOCS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
        migration_version=$(echo "$doc_dir" | sed 's/wildfly//')
        if grep -q "<module>docs/user-guides/migrations/wildfly${VERSION}/wildfly${migration_version}</module>" "$ROOT_POM"; then
            sed -i '' "/<module>docs\/user-guides\/migrations\/wildfly${VERSION}\/wildfly${migration_version}<\/module>/d" "$ROOT_POM"
        fi
    done
...
if ! $MIGRATIONS_ONLY; then
    if grep -q "jboss-server-migration-wildfly${VERSION}-server" "$ROOT_POM"; then
        remove_dependency "$ROOT_POM" "jboss-server-migration-wildfly${VERSION}-server"
    fi
fi
...
if [ -d "$MIGRATIONS_VERSION_DIR" ]; then
    for migration_dir in $(ls "$MIGRATIONS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
        ...
        if grep -q "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}" "$ROOT_POM"; then
            remove_dependency "$ROOT_POM" "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}"
        fi
    done
fi
...
if [ -d "$DOCS_VERSION_DIR" ]; then
    for doc_dir in $(ls "$DOCS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
        migration_version=$(echo "$doc_dir" | sed 's/wildfly//')
        if grep -q "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}-userguide" "$ROOT_POM"; then
            remove_dependency "$ROOT_POM" "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}-userguide"
        fi
    done
fi
...
if ! $MIGRATIONS_ONLY; then
    sed -i '' "/<!-- wfly ${VERSION} -->/d" "$DIST_POM"
fi

# Remove server dependency (unless skipping server)
if ! $MIGRATIONS_ONLY; then
    if grep -q "jboss-server-migration-wildfly${VERSION}-server" "$DIST_POM"; then
        remove_dependency "$DIST_POM" "jboss-server-migration-wildfly${VERSION}-server"
    fi
fi
...
if [ -d "$MIGRATIONS_VERSION_DIR" ]; then
    for migration_dir in $(ls "$MIGRATIONS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
        migration_version=$(echo "$migration_dir" | sed 's/wildfly//')
        if grep -q "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}" "$DIST_POM"; then
            remove_dependency "$DIST_POM" "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}"
        fi
    done
fi
...
if [ -f "$TOOL_POM" ]; then
    # Remove comment
    sed -i '' "/<!-- WFLY ${VERSION} -->/d" "$TOOL_POM"

    # Remove userguide dependencies
    if [ -d "$DOCS_VERSION_DIR" ]; then
        for doc_dir in $(ls "$DOCS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
            migration_version=$(echo "$doc_dir" | sed 's/wildfly//')
            if grep -q "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}-userguide" "$TOOL_POM"; then
                remove_dependency "$TOOL_POM" "jboss-server-migration-wildfly${migration_version}-to-wildfly${VERSION}-userguide"
            fi
        done
    fi
    ...
    if [ -d "$DOCS_VERSION_DIR" ]; then
        for doc_dir in $(ls "$DOCS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
            ...
            if grep -q "<id>WFLY${MIGRATION_MAJOR}.${MIGRATION_MINOR}toWFLY${TARGET_MAJOR}.${TARGET_MINOR}</id>" "$TOOL_POM"; then
                # Find line with the execution id
                EXEC_LINE=$(grep -n "<id>WFLY${MIGRATION_MAJOR}.${MIGRATION_MINOR}toWFLY${TARGET_MAJOR}.${TARGET_MINOR}</id>" "$TOOL_POM" | cut -d: -f1)
                if [ -n "$EXEC_LINE" ]; then
...
if [ -f "$MASTER_ADOC" ] && [ -d "$DOCS_VERSION_DIR" ]; then
    for doc_dir in $(ls "$DOCS_VERSION_DIR" 2>/dev/null | grep "^wildfly" || true); do
        ...
        sed -i '' "/link:migrations\/wildfly${migration_version}-to-wildfly${VERSION}\/index.html\[WildFly ${migration_version} to WildFly ${VERSION}\]/d" "$MASTER_ADOC"
    done
...
echo "========================================"
if $MIGRATIONS_ONLY; then
    echo "WildFly ${VERSION} migrations and docs have been removed from:"
